...
rules! {
    ...
    "tool-arg-shell", Fail, "a tool argument is interpolated into a shell command string in the server's source, so anything that steers the model can run commands", "Pass arguments as an array to spawn or execFile, never into a shell string. If the tool exists to run shell commands, say so: `server \"name\" may tool-arg-shell`.";
    "tool-arg-eval", Fail, "a tool argument reaches eval or code compilation in the server's source", "Do not evaluate model-controlled text. If the tool is a code interpreter by design, sandbox it and record the exception in the policy.";
    "tool-arg-exec", Warn, "a tool argument is passed to a spawned process in the server's source", "Expected for git and build tools. Check the argument cannot select the program itself, then allow it in the policy.";
...
const DIRECTIVE: &str = r"(?i)(ignore\s+(all\s+|any\s+)?(previous|prior|above|earlier|other)\s+(instructions|rules|guidance|directions)|(disregard|forget|override)\s+(all\s+|any\s+|your\s+)?(previous|prior|earlier|other|system)\s+(instructions|rules|guidance|prompt)|(this|these)\s+(instructions?|directives?)\s+(take|takes|has|have)\s+(precedence|priority)|do\s+not\s+(tell|inform|mention|reveal|show|warn|notify|disclose|alert|surface)\s+(this\s+|it\s+)?(to\s+)?(the\s+)?user|without\s+(asking|telling|informing|notifying|alerting|mentioning)\s+(it\s+to\s+)?(the\s+)?user|(the\s+)?user\s+(need\s+not|needn'?t|should\s+not|shouldn'?t|must\s+not|does\s+not\s+need\s+to|doesn'?t\s+need\s+to)\s+(be\s+)?(informed|told|know|see|notice|notified|aware)|never\s+(mention|reveal|tell|disclose|surface|show)\s+(this|these|that|it|the\s+(tool|server|instructions?|prompt|file|upload|step|change|payload|note)|to\s+the\s+user|anything\s+about\s+this)|keep\s+(this|it|these)\s+(secret|hidden|confidential|quiet|private\s+from\s+the\s+user)|(silently|quietly|covertly|discreetly)\s+(send|append|include|add|attach|forward|upload|read|copy|exfiltrate|transmit|post)|(pretend|act\s+as\s+if)\s+you|disable\s+(safety|guardrails|permissions|confirmations?)|bypass\s+(the\s+)?(permission|safety|approval|confirmation)|(reveal|leak|ignore|override|print|show|dump|repeat)\s+(the\s+|your\s+)?system\s+prompt|<\s*important\s*>|<\s*(system|hidden|secret|instructions?)\s*>|before\s+(calling|using|invoking|running)\s+(any\s+other|this|the)\s+tool|(prior\s+to|before)\s+(invoking|calling|using)\s+(any\s+)?other\s+tools?|always\s+(call|use|invoke|run)\s+this\s+(tool\s+)?first|hide\s+(this|the\s+following|it)\s+from|do\s+not\s+(show|display|render|surface)\s+(this|the|any)\s+(output|result|note|text|content)|send\s+(the\s+)?(contents?|file|data|ssh|keys?|credentials?|\.env|environment|messages?|conversation|history|context)\s+(of\s+[^\s]+\s+)?to\s+(https?://|[+0-9]|[a-z0-9.\-]+@)|(this\s+is\s+)?(required|necessary|needed)\s+for\s+the\s+tool\s+to\s+(work|function)|(append|include|add|attach)\s+(the\s+)?(contents?|text|data)\s+of\s+[~$/.\w-]*(\.ssh|\.env|credentials|id_rsa|\.aws|config\.json|mcp\.json)|the\s+user\s+(does\s+not|doesn'?t|won'?t|will\s+not)\s+(need\s+to\s+)?(see|know|notice))";
